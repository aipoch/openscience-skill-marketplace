...
node scripts/import-full-aipoch-assessments.mjs \
  --material /path/to/extracted/archive \
...
To reproduce with a safely extracted copy of the supplied archive:

```sh
node scripts/import-aipoch-audits.mjs --catalog /path/to/verified/marketplace.json \
  --material /path/to/extracted/archive --archive /path/to/open-science-skills-20260911.zip \
...
official CDN. Old clients ignore it. The JSON envelope contains `catalog` and
`signature`; the signature is the existing Ed25519 envelope over UTF-8
`JSON.stringify(catalog, null, 2) + '\n'`, verified against the client's independent
production pin. The new protocol discriminator is `openscience-aipoch-audits`,
schema version 1. Never accept the envelope's key as a trust anchor.

The catalog contains `catalog_revision`, `source_archive_sha256` and `entries`.
...
The protected, manually authorized publication workflow uploads immutable reports
before atomically writing the signed envelope and invalidating that CDN path.
